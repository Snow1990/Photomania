...
class PhotoViewerViewController: UIViewController, UIScrollViewDelegate, UIPopoverPresentationControllerDelegate, UIActionSheetDelegate {
    ...
    func loadPhoto(){
        let router = Five100px.Router.PhotoInfo(self.photoID, .Large)
    ...
    func downloadPhoto() {
        // 1
        Alamofire.request(Five100px.Router.PhotoInfo(photoInfo!.id, .XLarge)).validate().responseObject { (_, _, photoInfo: PhotoInfo?, error) -> Void in
